...
class binary_set {
    public:
        ...
        inline void clear() { this->set = std::vector<unsigned char>((this->capacity+7)/8, 0); }
        ...
        binary_set operator!() const {
            ...
            for (size_t i = 0; i < this->set.size(); i++) new_bs.set[i] = ~this->set[i];
        ...
        inline std::vector<size_t> sparse() const {
            ...
            for (auto p : *this) sparse.push_back(p);
            return sparse;
        }
        /** Get the string representation of the set */
        operator std::string() const {
            std::string str = "[";
            for (size_t i = 0; i < this->capacity; i++) str.append((*this)[i] ? "X" : " ");
            str.append("]");
            return str;
        }
        class iterator {
            public:
                iterator(const binary_set* _bs, size_t _s) : bs(_bs), c(_s) { if (this->c < this->bs->size() && !(*this->bs)[this->c]) ++(*this); }
                iterator& operator++() {
                    do {
                        ++this->c;
                ...
                size_t operator*() const { return this->c; }
                bool operator!=(const iterator &_it) const { return this->c != _it.c; }
            protected:
                const binary_set* bs;
                size_t c;
        };
        ...
        /** end iterator */
        iterator end() const { return iterator(this, this->capacity); }
    protected:
        size_t capacity;
...
class bs_searcher {
    public:
        ...
        inline void remove(size_t _v, const binary_set& _bs) {
            ...
            if (node == nullptr || !remove_value(node->values, _v)) return;
            ...
            while (!path.empty()) {
                ...
                if (is_empty_node(current_node->left) && is_empty_node(current_node->right) && 
        ...
        ~bs_searcher() {
            ...
            while (!rem_nodes.empty()) {
                ...
                rem_nodes.pop_front();
                if (node->left != nullptr) rem_nodes.push_back(node->left);
                if (node->right != nullptr) rem_nodes.push_back(node->right);
                delete node;
            }
        }
    private:
