...
public:
	...
	[[nodiscard]]
	bool has(size_t j) const { return (position[j] >= 0); }
	/** Clear content */
	void clear() {
	...
	void push(size_t j, score_type p, bool mantain_heap = true) {
		validate_element(j);
#if INTCHECK_PQ
		if (position[j] != -1)
	...
	void remove(size_t j, bool mantain_heap = true) {
#if INTCHECK_PQ
		if (position[j] < 0)
	...
	void change(size_t j, score_type p, bool mantain_heap = true) {
#if INTCHECK_PQ
		if (position[j] < 0)
			throw std::invalid_argument("Element wasn't in the priority queue.");
#endif
...
private:
	...
	void validate_element(size_t j) const {
#if INTCHECK_PQ
		if (j >= n)
