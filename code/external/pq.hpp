...
class priority_queue {
    ...
    explicit operator std::string() const {
        ...
        for (size_t k = 0; k < cnt; k++)
            repr.append("\t")
                ...
                .append("\t")
                .append(std::to_string(data[k]))
                .append("\t")
                .append(std::to_string(prior[data[k]]))
                .append("\n");
        return repr;
    }
   ...
   private:
    ...
    /**
     * Moves the gap up from the current position gap to the proper place for a
     * priority of value p The final position of the gap is stored in gap
     */
    ...
    void remove_at(int gap, const bool mantain_heap) {
        ...
        position[data[gap]] = -1;
        if (gap == static_cast<int>(last)) return;
    ...
    void check_empty() const {
        if (empty()) throw std::domain_error("The priority queue is empty");
    }
