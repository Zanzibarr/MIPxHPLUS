...
static inline auto operator|=(utilz::BinarySet& bs, const std::vector<unsigned int>& vec) -> utilz::BinarySet& {
...
static inline auto bs_contains(const utilz::BinarySet& bs, const std::vector<unsigned int>& vec) -> bool {
    return std::ranges::all_of(vec, [&bs](unsigned int val) { return bs[val]; });
}

static inline auto bs_intersects(const utilz::BinarySet& bs, const std::vector<unsigned int>& vec) -> bool {
...
class bs_searcher {
   ...
   public:
     ...
     */
    void add(unsigned int value, const utilz::BinarySet& bs) {
     ...
     * specified in constructor
     */
    auto remove(unsigned int value, const utilz::BinarySet& bs) -> bool {
        validate_capacity(bs);
     ...
     */
    [[nodiscard]]
    auto find_subsets(const utilz::BinarySet& bs) const -> std::vector<unsigned int> {
        validate_capacity(bs);
   ...
   private:
    ...
    void validate_capacity(const utilz::BinarySet& bs) const {
