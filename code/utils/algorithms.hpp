...
static inline std::vector<std::vector<T>> find_cycles_unweighted(
    ...
    std::vector<unsigned int> vertices(graph.size());
    std::iota(vertices.begin(), vertices.end(), 0);
    // Sort by degree to explore low-degree nodes first (might be helpful for early-pruning in sparse graphs)
    std::sort(vertices.begin(), vertices.end(), [&](unsigned int a, unsigned int b) { return graph[a].size() < graph[b].size(); });
...
static inline std::vector<std::vector<T>> find_cycles_weighted_lessthan1(
    ...
    // Sort vertices by degree for better pruning in sparse graphs
    std::vector<unsigned int> vertices(graph.size());
    std::iota(vertices.begin(), vertices.end(), 0);
    std::sort(vertices.begin(), vertices.end(), [&](unsigned int a, unsigned int b) { return graph[a].size() < graph[b].size(); });
