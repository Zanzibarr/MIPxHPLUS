...
/** @brief Pair hash for edge_labels and edge_weights sets, where the key is the pair (from, to) of the edge */
struct pair_hash {
    auto operator()(const std::pair<unsigned int, unsigned int>& p) const -> std::size_t {
        return std::hash<unsigned int>()(p.first) ^ (std::hash<unsigned int>()(p.second) << 1);
    }
};

/** @brief DFS algorithm to look for a cycle: start from the start node but the cycle returned (if any) is not guaranteed to contain the start
 * vertex... */
static inline void cycle_dfs(const std::vector<std::vector<unsigned int>>& graph,
                             ...
                             unsigned int start_vertex, std::vector<std::vector<bool>>& removed_edges, std::vector<bool>& removed_nodes,
