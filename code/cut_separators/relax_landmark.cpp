...
[[nodiscard]]
static inline auto get_r3_violated_landmark(const hplus::instance& inst, const std::vector<std::vector<network_edge>>& graph)
    -> std::vector<unsigned int> {
...
[[nodiscard]]
auto relax_cuts::get_r3_violated_landmark(const hplus::execution& exec, const hplus::instance& inst, std::vector<double> relax_point,
...
auto relax_cuts::get_lmcut_violated_landmarks(const hplus::execution& /*exec*/, const hplus::instance& inst, const std::vector<double>& relax_point,
    ...
    const auto& landmarks = lmcut.fract_separation(actions_weights, hmax::hmax_arbitrary);
...
auto relax_cuts::add_lm_cut(CPXCALLBACKCONTEXTptr context, const hplus::execution& exec, const hplus::instance& inst,
    ...
    if (exec.fract_cuts.find('m') != std::string::npos) {
        const auto& [found, landmark] = get_r3_violated_landmark(exec, inst, relax_point, n_act_in_lm, n_lm);
        if (found) {
            landmarks.push_back(std::move(landmark));
        }
    }
    if (exec.fract_cuts.find('l') != std::string::npos) {
        const auto& [found, landmarks_lmcut] = get_lmcut_violated_landmarks(exec, inst, relax_point, n_act_in_lm, n_lm);
        if (found) {
