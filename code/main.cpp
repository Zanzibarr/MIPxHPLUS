...
static void parse_cli(const int& argc, const char** argv, hplus::environment& env) {
    ...
    args::ValueFlag<std::string> algorithm(parser, "alg_name", "Specify the algorithm to use (def: ve; options: heur, tl, ve, lm).", {"a", "alg"},
                                           HPLUS_CLI_ALG_VE);
    ...
    args::ValueFlag<std::string> heur(parser, "heur_name",
                                      ...
                                      "hmax, hadd).",
                                      {"heur"}, "hadd");
    args::ValueFlag<bool> warmstart(parser, "0/1", "Using the heuristic (if computed) as warm start (def: true; options: 0 (false), 1 (true)).",
                                    {"ws"}, true);
    ...
    args::ValueFlag<bool> minimal_lm(
        parser, "0/1", "Using minimal landmarks as lazy constraints in the lm model (def: true; options: 0 (false), 1 (true)).", {"mlm"}, true);
    args::ValueFlag<bool> complete_lm(
        parser, "0/1", "Using complete landmarks as lazy constraints in the lm model (def: true; options: 0 (false), 1 (true)).", {"clm"}, true);
    args::ValueFlag<bool> sec(parser, "0/1", "Using S.E.C. as lazy constraints in the lm model (def: true; options: 0 (false), 1 (true)).", {"sec"},
                              true);
