#include "../include/dynamic_model_small.hpp"
...
static int CPXPUBLIC HPLUS_cpx_candidate_callback(CPXCALLBACKCONTEXTptr context, CPXLONG context_id, void* user_handle) {
    ...
    my::binary_set reachability_gap(n_var_presim);
    ...
    // build constraint
    int nnz_i = 0;
    int nnz = actions_unreachable.size() + forced_actions.size();
    double rhs = 0;
...
void HPLUS_cpx_build_dynamic_small(CPXENVptr& env, CPXLPptr& lp) {
    ...
    for (auto act_i : remaining_actions) {
        for (auto var_i : actions[act_i].get_pre_sparse()) if (remaining_var_set[var_i]) {
            ...
            val[1] = -1;
            my::assert(!CPXaddrows(env, lp, 0, 1, 2, &rhs_0, &sense_l, &begin, ind, val, nullptr, nullptr), "CPXaddrows (newc) failed.");
        }
    }
    ...
    for (auto var_i : remaining_variables) {
        ...
        my::assert(!CPXaddrows(env, lp, 0, 1, nnz, &rhs_0, &sense_l, &begin, ind, val, nullptr, nullptr), "CPXaddrows (newc) failed.");
    }
