...
HPLUS_variable::HPLUS_variable(const unsigned int range, const std::string& name, const std::vector<std::string>& val_names) {
    ...
    for (unsigned int i = 0; i < this -> range; i++) this -> val_names[i] = val_names[i];
...
HPLUS_action::HPLUS_action(my::BitField* pre_bf, my::BitField* eff_bf, const unsigned int cost, const std::string& name) {
    ...
    #if HPLUS_INTCHECK
    MYASSERT(pre_bf != nullptr);
    MYASSERT(eff_bf != nullptr);
    #endif
...
HPLUS_instance::~HPLUS_instance() {
    ...
    for (unsigned int i = 0; i < this -> n_var; i++) { MYASSERT(this -> variables != nullptr && this -> variables[i] != nullptr); delete this -> variables[i]; }
    MYDELL(this -> variables);
    for (unsigned int i = 0; i < this -> n_act; i++) { MYASSERT(this -> actions != nullptr && this -> actions[i] != nullptr); delete this -> actions[i]; }
    MYDELL(this->actions);
...
void HPLUS_instance::update_best_solution(const std::vector<unsigned int>& solution, const unsigned int cost) {
    ...
    for (unsigned int i = 0; i < this -> bf_size; i++) if (this -> initial_state -> operator[](i)) feas_checker.set(i);
    for (unsigned int i = 0; i < nact; i++) {
        MYASSERT(solution[i] < this -> n_act);
        MYASSERT(!dbcheck[solution[i]]);
        dbcheck[solution[i]] = true;
        MYASSERT(feas_checker.validate(this -> actions[solution[i]] -> get_pre()));
        for (unsigned int j = 0; j < this -> bf_size; j++) if (this -> actions[solution[i]] -> get_eff() -> operator[](j)) feas_checker.set(j);
    ...
    for (unsigned int i = 0; i < nact; i++) this -> best_solution[i] = solution[i];
    this -> best_nact = nact;
    this -> best_cost = cost;
...
void HPLUS_instance::get_best_solution(std::vector<unsigned int>* solution, unsigned int* cost) const {
    ...
    #if HPLUS_INTCHECK
    MYASSERT(solution != nullptr);
    MYASSERT(cost != nullptr);
    #endif

    solution -> clear();
    (*solution) = std::vector<unsigned int>(this -> best_nact);
    for (unsigned int i = 0; i < this -> best_nact; i++) (*solution)[i] = this -> best_solution[i];
    *cost = this -> best_cost;
...
void HPLUS_instance::print_best_sol() const {
    ...
    for(unsigned int i = 0; i < this -> best_nact; i++) HPLUS_env.logger.print("(%s)", this -> actions[this -> best_solution[i]] -> get_name() -> c_str());
...
void HPLUS_instance::parse_inst_file(std::ifstream* ifs) {
    ...
    this -> n_var = std::stoi(line);
    this -> variables = new const HPLUS_variable*[this -> n_var];
    this -> bf_size = 0;
    for (unsigned int var_i = 0; var_i < this -> n_var; var_i++) {
    ...
    for (unsigned int var_i = 0, c = 0; var_i < this -> n_var; c += this -> variables[var_i] -> get_range(), var_i++) {
    ...
    for (unsigned int i = 0; i < ngoals; i++) {
        ...
        unsigned int c = 0;
        for (unsigned int j = 0; j < var; c += this -> variables[j] -> get_range(), j++){}
    ...
    for (unsigned int act_i = 0; act_i < this -> n_act; act_i++) {
        ...
        for (unsigned int pre_i = 0; pre_i < n_pre; pre_i++) {
            ...
            unsigned int value = stoi(tokens[1]);
            unsigned int c = 0;
            for (unsigned int i = 0; i < var; c += this -> variables[i] -> get_range(), i++){}
            act_pre -> set(c + value);
        }
        std::getline(*ifs, line);   // number of effects
        MYASSERT(my::isint(line, 0));
        unsigned int n_eff = stoi(line);
        auto* act_eff = new my::BitField(this -> bf_size);
        ...
        MYASSERT(my::isint(line));
        unsigned int cost = 1;
        if (this -> use_costs) cost = stoi(line);
