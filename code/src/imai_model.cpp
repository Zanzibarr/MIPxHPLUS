...
void HPLUS_cpx_build_imai(CPXENVptr& env, CPXLPptr& lp, HPLUS_instance& inst) {
    ...
    for (unsigned int act_i = 0; act_i < nact; act_i++) {
        objs[act_i] = 0;
        lbs[act_i] = (*inst.fixed_act_timestamps)[act_i] >= 0 ? (*inst.fixed_act_timestamps)[act_i] : 0;
        ubs[act_i] = (*inst.fixed_act_timestamps)[act_i] >= 0 ? (*inst.fixed_act_timestamps)[act_i] : nact-1;       //[ ]: Tighter bound
        types[act_i] = 'I';
    }
    curr_col += nact;
