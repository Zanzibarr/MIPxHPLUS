...
static inline void parse_cli(const int& _argc, const char** _argv, hplus::environment& _e) {
    ...
    args::ValueFlag<std::string> runname(parser, "run name", "Specify the name of the run that will be shown in the log file as well as the log/lp files generated by cplex.", {"run"});
    ...
    if (!input_file) {
        ...
        exit(1);
    } else _e.input_file = args::get(input_file);
    struct stat buffer{};
    if(stat((_e.input_file).c_str(), &buffer) != 0) {
        std::cerr << "Failed to open input file.\n";
        exit(1);
    }
    if (algorithm) _e.alg = args::get(algorithm);
    ...
    if (runname) _e.run_name = args::get(runname);
