...
static inline void init(hplus::statistics& _s) {
	_s = (hplus::statistics){ .parsing = 0, .optimization = 0, .heuristic = 0, .build = 0, .callback = 0, .execution = 0, .total = 0 };
...
static inline void parse_cli(const int& _argc, const char** _argv, hplus::environment& _e) {
	...
	args::ValueFlag<std::string>  heur(parser, "heuristic",
		 "Specify which heuristic to compute before running cplex (default: 'greedycxe', options: [greedycost, greedycxe, rand, "
		  "randr, hmax, hadd, relax, local-<one of the other heuristics>], 'none' to not compute an heuristic).",
		 { "heur" });
	...
	args::ValueFlag<std::string>  logname(
		 parser, "log name", "Specify the name of the log file (will be located inside the logs/AAA_output_logs folder.); if this is missing no log will be written.", { "l", "log" });
	args::ValueFlag<std::string> runname(
		parser, "run name", "Specify the name of the run that will be shown in the log file as well as the log/lp files generated by cplex.", { "run" });
...
static inline void show_info(const hplus::instance& _i, const hplus::environment& _e, const logger& _l) {
	...
	if (!_e.run_name.empty())
		_l.print("Run name:                              %20s.", _e.run_name.c_str());
	_l.print("Verbose parameter:                               %10d.", HPLUS_VERBOSE);
	_l.print("Warnings parameter:                              %10d.", HPLUS_WARN);
	_l.print("Integrity checks parameter:                      %10d.", HPLUS_INTCHECK);
	...
	_l.print("Algorithm:                                       %10s.", _e.alg.c_str());
	_l.print("Problem simplification:                          %10s.", _e.problem_opt ? "Y" : "N");
	if (_e.alg == HPLUS_CLI_ALG_IMAI)
		_l.print("Tighter bounds on variable timestamps:           %10s.", _e.imai_tight_bounds ? "Y" : "N");
	if (_e.heur != "none")
		_l.print("Heuristic:                                       %10s.", _e.heur.c_str());
	if (_e.using_cplex)
		_l.print("Warm start:                                      %10s.", _e.warm_start ? "Y" : "N");
...
static inline void run(hplus::instance& _i, hplus::environment& _e, hplus::statistics& _s, const logger& _l) {
	...
	try {
		...
		if (static_cast<double>(_e.time_limit) > _e.timer.get_time()) {
			_ASSERT(!CPXsetdblparam(env, CPXPARAM_TimeLimit, static_cast<double>(_e.time_limit) - _e.timer.get_time()));
		} else
			return;
	...
	if (_e.timer.get_time() >= _e.time_limit) {
		switch (_e.exec_s) {
			case exec_status::START:
				_l.print("Reached time limit before the program could read the instance file.");
				break;
