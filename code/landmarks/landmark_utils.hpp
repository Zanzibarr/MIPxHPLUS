...
inline void landmark_minimalization(const hplus::instance& inst, std::vector<unsigned int>& landmark, std::vector<unsigned int> unapplicable,
    ...
    //     // Looking at all exit conditions of the previous while loop, the watch_pre of the goal is up-to-date here
    ...
    //     // watch_pre and watching vectors, because if watch_pre of an (unachievable) action changed, it can't have changed to a fact that was
    //     achieved
    //     // pre-simulation, so the new watch_pre is still not achieved in the pre-simulation envorinment that we are reversing to (same with
    //     watching,
