...
void int_lm_sep::landmark_minimalization_greedy(const hplus::instance& inst, std::vector<unsigned int>& landmark,
    ...
    for (const auto& act_i : landmark) {
        ...
        if (!set_contains(reachable_state, inst.actions[act_i].pre_sparse)) {
