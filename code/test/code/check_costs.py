...
def main():
    ...
    if input(f"Run name will be '{run_results_file}'\nInsert y if it's correct: ") != "y": exit(0)

    with open(run_results_file, "r") as f:
        run_results = json.loads(f.read())
        
    for file in os.listdir("../../../local/fast_downward_baseline"):
        file_path = os.path.abspath(os.path.join("../../../local/fast_downward_baseline", file))
        with open(file_path, "r") as f:
            content = f.read()
        
        if file not in run_results["results"]: continue
        ...
        if inst_res["status"] < 0: continue

        elif (
                inst_res["status"] == 0 and (inst_res["fcost"] < bound[0] or inst_res["fcost"] > bound[1])
            ) or (
                inst_res["status"] == 1 and content.splitlines()[0] != "0.0 1e+20"
            ) or (
                inst_res["status"] == 2 and inst_res["hcost"] < bound[0]
            ):
            errors += 1
            errors_names += f"{file}\n"
