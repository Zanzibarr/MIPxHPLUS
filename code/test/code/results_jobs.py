    ...
    for file in os.listdir(logsdir):
        ...
        if runsum["results"][instance_name]["status"] > -2:
            ...
            if runsum["results"][instance_name]["status"] > -1:
                ...
                opt_time = float(content.partition(">>  Problem simplification time")[2].partition("s")[0].strip())
                ...
                callback_time = float(content.partition(">>  CPLEX callback time")[2].partition("s")[0].strip())
                cplex_time = float(content.partition(">>  CPLEX execution time")[2].partition("s")[0].strip())
                ...
                if runsum["results"][instance_name]["status"] == 0:
                    fcost = int(content.partition("Solution cost:")[2].partition("\n")[0].strip())
                    runsum["results"][instance_name]["fcost"] = fcost
                if runsum["results"][instance_name]["status"] not in (1, 3) and "Heuristic:" in content.partition("Problem simplification:")[2].partition("Time limit:")[0]:
                    hcost = int(content.partition("Updated best solution - Cost:")[2].partition(".\n")[0].strip())
